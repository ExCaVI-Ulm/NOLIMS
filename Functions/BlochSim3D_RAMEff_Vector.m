...
%           -u: loop variable
%   Output:
%           -BlochM0: Transversal magnetization after TX pulse
    ...
    Mlong_init = zeros(settings.general.RAM_StepsPhaseEnc, size(Bvec_RF,2));
    Minit = zeros(settings.general.RAM_StepsPhaseEnc, size(Bvec_RF,2), 3);
    Mvec_pulse = zeros(settings.general.RAM_StepsPhaseEnc,4, size(Bvec_RF,2));
    Mvec_pulse_2 = zeros(settings.general.RAM_StepsPhaseEnc,3, size(Bvec_RF,2));
    ...
    B_vec_rot = reshape(Bvec_RF, settings.general.RAM_StepsPhaseEnc,settings.signal.matrixsize_signal,4); 
    ...
    Mlong_init_Rot_straight = reshape(Mlong_init, settings.general.RAM_StepsPhaseEnc, size(Bvec_RF,2)); 
    Minit_straight = reshape(Minit, settings.general.RAM_StepsPhaseEnc, size(Bvec_RF,2), 3);
    %T1 effect
   ...
   angle_loc =  settings.TX.AngleRF_Theta;
    ...
    B1x_vec = repmat(B1x_vec, [1 1 settings.signal.matrixsize_signal]);
    B1y_vec = repmat(B1y_vec, [1 1 settings.signal.matrixsize_signal]);
    B1z_vec = repmat(B1z_vec, [1 1 settings.signal.matrixsize_signal]);
    ...
    B1x_v_straightSpat = reshape(B1x_vec, settings.general.RAM_StepsPhaseEnc, length(time_Bloch), settings.signal.matrixsize_signal); 
    B1y_v_straightSpat = reshape(B1y_vec, settings.general.RAM_StepsPhaseEnc, length(time_Bloch), settings.signal.matrixsize_signal); 
    B1z_v_straightSpat = reshape(B1z_vec, settings.general.RAM_StepsPhaseEnc, length(time_Bloch), settings.signal.matrixsize_signal); 

   for ll=1:settings.general.RAM_StepsPhaseEnc
            parfor j =1:(size(Bvec_RF,2))%par
                ...
                for u=2:length(time_Bloch)
                    ...
                    Btot_x = squeeze(B1x_v_straightSpat(ll,u-1,j)) + squeeze(B_vec_rot(ll,j,1));
                     A = [0*-1/T2_straight_rot(j),  gamma_loc*Btot_z,  -gamma_loc*Btot_y,       0; ...        
                          -gamma_loc*Btot_z,   0*-1/T2_straight_rot(j),  gamma_loc*Btot_x ,          0; ...
                           gamma_loc*Btot_y,   -gamma_loc*Btot_x,     0*-1/T1_straight_rot(j) ,   0*Mlong_init_Rot_straight(ll,j)/(T1_straight_rot(j)); ...
                            0                     0                            0                        1];
    ...
    for jj = 1:3
        BlochM0(:,:,jj) = reshape(squeeze(Mvec_pulse_2(:,jj,:)), settings.general.RAM_StepsPhaseEnc, settings.signal.matrixsize_signal); 
    end
end
